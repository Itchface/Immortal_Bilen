...
float svang_L(float pV_45, float pH_45){
float V,SV,x,y;

//300 är ett hittepåvärde för ca 3cm från sensorena till bilens mitt.
x=pV_45+600;
y=pH_45+600;
...
float svang_R(float pV_45, float pH_45){
  float V,SV,x,y;
...
int hast=150;
 //Motorfunktionen
  ...
  //Styrfunktionen
  while(left_45.read()<right_45.read())
  {

float L,R,funkR,angle;

    funkR=svang_R(left_45.read(),right_45.read());
      angle=map(funkR,30,170  ,90,170);
  myservo.write(angle);
  
    //Serial.println("högersväng");
  }
 
  while(right_45.read()<left_45.read())
  {
    float L,R,funkL, angle;

    funkL=svang_L(left_45.read(),right_45.read());
    angle=map(funkL,30,170,16,90);
